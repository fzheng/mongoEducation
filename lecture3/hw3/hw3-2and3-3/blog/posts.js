...
function PostsDAO(db) {
    ...
    this.insertEntry = function (title, body, tags, author, callback) {
        ...
        posts.insert(post, function(err) {
          if (err) callback(Error("insert error in PostsDAO"), null);
    ...
    this.addComment = function(permalink, name, email, body, callback) {
        ...
        posts.findOne({'permalink': permalink}, function(err, post) {
            "use strict";
            if (err) return callback(err, null);
            var theComment = post.comments;
            theComment.push(comment);
            posts.update({'permalink': permalink}, {$set: {comments: theComment}}, function(err) {
              if (err) return callback(err, null);
